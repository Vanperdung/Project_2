...
bool cJson_process(char *data, cmd_object_t *cmd_object)
{
    ...
    cJSON_ArrayForEach(cur_element, root)
    {
        if(cur_element->string)
        {
            ...
            if(strcmp(cur_string, "type_node") == 0)
                memcpy(cmd_object->type_node, cur_element->valuestring, strlen(cur_element->valuestring) + 1);
            if(strcmp(cur_string, "mac") == 0)
                sprintf(cmd_object->mac_addr, "%02x%02x%02x%02x%02x%02x", cur_element->valuestring[0],
            ...
            if(strcmp(cur_string, "node_id") == 0)
            ...
            switch(scan_result->scan_rst.search_evt)
            {
                case ESP_GAP_SEARCH_INQ_RES_EVT:
                    ...
                    if(adv_node_data[0] == '{' && adv_node_data[adv_node_len - 1] == '}')
                    {
                        ...
                        if(cJson_process((char*)adv_node_data, &cmd_object) == true)
                        {
                            if(strstr(cmd_object.type_node, "switch") != NULL)
                            {
                                ...
                                {
                                    connect = true;
                                    cJson_process((char*)rsp_node_data, &cmd_object);
                                    ESP_LOGI(TAG, "Searched type_node: %s, MAC: %s, node_id: %d", cmd_object.type_node, cmd_object.mac_addr, cmd_object.node_id);
                                    printf("\r\n");
