            ...
            switch(scan_result->scan_rst.search_evt)
            {
                case ESP_GAP_SEARCH_INQ_RES_EVT:
                    ...
                    if(adv_node_data[0] == '{' && adv_node_data[adv_node_len - 1] == '}')
                    {
                        ...
                        if(ble_cJSON_process((char*)adv_node_data, &ble_object) == true)
                        {
                            if(strstr(ble_object.type_node, "switch") != NULL)
                            {
                                if(connect_a == false)
                                {
                                    connect_a = true;
                                    ble_cJSON_process((char*)rsp_node_data, &ble_object);
                                    ESP_LOGI(TAG, "Searched type_node: %s, MAC: %s, node_id: %d", ble_object.type_node, ble_object.mac_addr, ble_object.node_id);
