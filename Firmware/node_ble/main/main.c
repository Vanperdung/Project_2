...
static const char *NODE_TYPE = "switch";
static const int TOTAL_SWITCH = 4;
...
static esp_ble_adv_params_t node_adv_params = {
    .adv_int_min = 0x20,
    .adv_int_max = 0x40,
...
static esp_ble_adv_data_t node_adv_data = {
    .include_name = true,
    .set_scan_rsp = false,
...
static esp_ble_adv_data_t node_rsp_data = {
    .include_name = true,
    .set_scan_rsp = true,
    .include_txpower = true,
    .min_interval = 0x0006,
...
static void gap_event_handler(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param)
{
    ...
    case ESP_GAP_BLE_ADV_DATA_SET_COMPLETE_EVT:
        ...
        if (adv_config_done == 0)
        {
            ...
            esp_ble_gap_start_advertising(&node_adv_params);
        }
        break;
...
static void gatts_profile_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    ...
    case ESP_GATTS_REG_EVT:
    {
        ...
        sprintf(device_name, "$,%s,%s,%d,*", NODE_TYPE, (char *)mac_hex, TOTAL_SWITCH);
        ret = esp_ble_gap_set_device_name(device_name);
        if (ret)
        {
            ESP_LOGE(PROFILE_1_TAG, "Set device name failed, error code = %x", ret);
        }
        ret = esp_ble_gap_config_adv_data(&node_adv_data);
        if (ret)
        {
            ESP_LOGE(PROFILE_1_TAG, "Config adv data failed, error code = %x", ret);
        }
        adv_config_done |= ADV_CONFIG_FLAG;
        ret = esp_ble_gap_config_adv_data(&node_rsp_data);
        if (ret)
        {
